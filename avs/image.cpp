...
unsigned char* GetVideoBuffer(PDEVICE_EXTENTION pdx, ULONG *buffSize)
{
	unsigned char* videoBuffer = NULL;
	KIRQL oldIRQL;
	PUSER_BUFFER_ENTRY pCurrentBuffer;
	KeAcquireSpinLock(&pdx->spinLock, &oldIRQL);
	switch (pdx->listCount)
	{
	case 0:
		break;
	case 1:
		// Если один в очереди кадр
		// Получаю кадр
		pCurrentBuffer = (PUSER_BUFFER_ENTRY)CONTAINING_RECORD(pdx->listHead.Blink, USER_BUFFER_ENTRY, link);
		// Получаю его размер
		*buffSize = pCurrentBuffer->buffSize;
		// Получаю буфер
		videoBuffer = (unsigned char*)pCurrentBuffer->userData;
		break;
	default:
		// Если более одного в очереди
		// получаю последний кадр
		pCurrentBuffer = (PUSER_BUFFER_ENTRY)CONTAINING_RECORD(pdx->listHead.Blink, USER_BUFFER_ENTRY, link);
		// Получаю его размер
		*buffSize = pCurrentBuffer->buffSize;
		// Получаю буфер
		videoBuffer = (unsigned char*)pCurrentBuffer->userData;
		// цикл, очистки, пока первый элемент не равен последнему
		while (pdx->listHead.Flink != pdx->listHead.Blink)
		{
			// удаляю все лишние кадры, оставляя только текущий
			PLIST_ENTRY entryToDelete = RemoveHeadList(&pdx->listHead);
			PUSER_BUFFER_ENTRY pBufferToDelete = (PUSER_BUFFER_ENTRY)CONTAINING_RECORD(entryToDelete, USER_BUFFER_ENTRY, link);
			// Освободождение памяти
			ExFreePool(pBufferToDelete->userData);
			ExFreePool(pBufferToDelete);
			pdx->listCount--;
		}
		break;
	}
...
void CImageSynthesizer::ShowMyStream()
{
	ULONG buffSize;
	videoBuffer = GetVideoBuffer((PDEVICE_EXTENTION)DeviceObject->DeviceExtension, &buffSize);

	if (videoBuffer != NULL)
	{
		GetImageLocation(0, 0);
		RtlCopyMemory(m_Cursor, videoBuffer, buffSize);
// 		for (ULONG pos = 0; pos < buffSize; pos++)
// 		{
// 			*m_Cursor++ = videoBuffer[pos];
// 		}
	}
